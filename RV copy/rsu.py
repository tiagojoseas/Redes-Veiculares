...
def get_next_node_traffic_jam(node_x, node_y,radius_s, radius_b):
    ...
    list(cars_neigh_connected.values())[0]

    dist = ((node_x-list(cars_neigh_connected.values())[0].get(FIELD_POS_X))**2+(node_y-list(cars_neigh_connected.values())[0].get(FIELD_POS_X))**2)**(1/2) #calcualr a distancia do proprio no
    ipCarAux = list(cars_neigh_connected.values())[0].get(FIELD_ORIGIN)
